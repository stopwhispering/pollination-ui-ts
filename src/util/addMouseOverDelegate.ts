...
export default function addMouseOverDelegate(_: string) {
        ...
        let oHBox = oAvatar.getParent() as HBox;
        let oCustumListItem = oHBox.getParent() as CustomListItem;
        let oList = oCustumListItem.getParent() as List;
        let oScrollContainer = oList.getParent() as ScrollContainer;
        let oHbox2 = oScrollContainer.getParent() as HBox;
        const oVBox = oHbox2.getParent() as VBox;
        let oDynamicPage = oVBox.getParent() as DynamicPage;
        const oPage = oDynamicPage.getParent() as DynamicPage;
        const oApp = oPage.getParent() as App;
        const oXMLView = oApp.getParent() as XMLView;
