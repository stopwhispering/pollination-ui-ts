sap.ui.define(["sap/ui/base/ManagedObject", "../interfaces/enums"], function (ManagedObject, ___interfaces_enums) {
  const PollinationStatus = ___interfaces_enums["PollinationStatus"];
  /**
   * @namespace pollination.ui.model
   */
  const formatter = ManagedObject.extend("pollination.ui.model.formatter", {
    html_for_count_stored_pollen_containers: function _html_for_count_stored_pollen_containers(iCount) {
      if (!iCount || iCount == 0) {
        return '';
      }
      if (iCount == 1) {
    ...
    html_for_pollination_status_indicator: function _html_for_pollination_status_indicator(pollination_status, ongoing, reverse) {
      if (ongoing) {
    ...
    html_for_active_florescence_dates: function _html_for_active_florescence_dates(inflorescence_appearance_date, first_flower_opening_date, last_flower_closing_date, florescence_status) {
    ...
    html_for_pollination_probability: function _html_for_pollination_probability(probability_pollination_to_seed) {
      // dates in format '%Y-%m-%d', e.g. '2022-11-16'
